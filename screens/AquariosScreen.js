...
const AquariosScreen = () => {
      ...
      try {
        ...
        const produtosList = querySnapshot.docs
          ...
          .filter(
            produto =>
              produto.categoria &&
              produto.categoria.toLowerCase() === 'aquarios'
          );
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.titulo}>🐠 Aquários à Venda</Text>
      <FlatList
        data={produtos}
...
const createStyles = (scheme) =>
  StyleSheet.create({
    container: {
      flex: 1,
      padding: 20,
      backgroundColor: scheme === 'dark' ? '#000' : '#f0f8ff',
    },
    loadingContainer: {
      flex: 1,
    ...
    nomeProduto: {
      textAlign: 'center',
      fontSize: 15,
      color: scheme === 'dark' ? '#eee' : '#00796b',
      fontWeight: '600',
    },
  });

export default AquariosScreen;
